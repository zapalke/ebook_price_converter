...
class EbookPriceConverter:
    def __init__(self, ebook_list_path, result_save_path, country="US"):
        ...
        try:
            ...
            self.exchange_rates = pd.DataFrame(
                columns=["date", "currency", "rate", "tableNo"]
            )
    ...
    def read_data(self):
        ...
        try:
            self.ebooks_to_check = pd.read_csv(
                self.ebook_list_path, header=0
            ).to_numpy()
            print(
                f"Found {len(self.ebooks_to_check)} ebooks in file {self.ebook_list_path}"
            )
    ...
    def get_ebook_data_from_itunes(self):
        ...
        for ebook in tqdm(range(len(self.ebooks_to_check)), ncols=60):
            ...
            search_term = (
                f"{ebook_author.replace(' ','+')}+{ebook_name.replace(' ','+')}"
            )
            ebook_itunes_request = requests.get(
                f"https://itunes.apple.com/search?term={search_term}&country={self.country}&media=ebook"
            )
            if ebook_itunes_request.status_code != 200:
                error_msg_list.append(
                    f"Request for {ebook_author} {ebook_name} failed with status code {ebook_itunes_request.status_code}"
                )
                ebook_errors += 1
                continue
            ...
            self.ebook_data.append(
                {
                    ...
                    "date": datetime.strftime(
                        datetime.strptime(
                            ebook_itunes_data["releaseDate"], "%Y-%m-%dT%H:%M:%SZ"
                        ),
                        "%Y-%m-%d",
                    ),
    ...
    def check_if_exchange_rate_is_already_saved(self, ebook):
        ...
        if (
            self.exchange_rates[
                (self.exchange_rates["date"] == self.ebook_data[ebook]["date"])
                & (self.exchange_rates["currency"] == self.ebook_data[ebook]["curr"])
            ].shape[0]
            > 0
        ):
            self.ebook_data[ebook]["fromNBP"]["rate"] = self.exchange_rates[
                (self.exchange_rates["date"] == self.ebook_data[ebook]["date"])
                & (self.exchange_rates["currency"] == self.ebook_data[ebook]["curr"])
            ]["rate"].values[0]
            self.ebook_data[ebook]["fromNBP"]["tableNo"] = self.exchange_rates[
                (self.exchange_rates["date"] == self.ebook_data[ebook]["date"])
                & (self.exchange_rates["currency"] == self.ebook_data[ebook]["curr"])
            ]["tableNo"].values[0]
            self.ebook_data[ebook]["fromNBP"]["pricePLN"] = (
                self.ebook_data[ebook]["price"]
                * self.ebook_data[ebook]["fromNBP"]["rate"]
            )
            return True
        else:
            return False
    ...
    def get_exchange_rates_data_and_convert_price_to_PLN(self):
        ...
        for ebook in tqdm(range(len(self.ebook_data)), ncols=60):
            ...
            if release_date < datetime(2002, 1, 2):
                ...
                NBP_erros += 1
                error_msg_list.append(
                    f'Cannot get exchange rates for ebook {self.ebook_data[ebook]["name"]} released on {release_date}. NBP API does not provide data for dates before 2002-01-02.'
                )
                continue
                ...
                nbp_request = requests.get(
                    f'http://api.nbp.pl/api/exchangerates/rates/A/{self.ebook_data[ebook]["curr"]}/{self.ebook_data[ebook]["date"]}/'
                )
                if nbp_request.status_code != 200:
                    ...
                    for i in range(1, max_days_ago):
                        previous_date = datetime.strftime(
                            release_date - timedelta(days=i), "%Y-%m-%d"
                        )
                        if release_date - timedelta(days=i) < datetime(2002, 1, 2):
                            NBP_erros += 1
                            error_msg_list.append(
                                f'Cannot get exchange rates for ebook {self.ebook_data[ebook]["name"]} released on {release_date}. NBP API does not provide data for dates before 2002-01-02.'
                            )
                            ...
                            nbp_request = requests.get(
                                f'http://api.nbp.pl/api/exchangerates/rates/A/{self.ebook_data[ebook]["curr"]}/{previous_date}/'
                            )
                            if nbp_request.status_code == 200:
                ...
                if nbp_request.status_code != 200:
                    error_msg_list.append(
                        f'Cannot get exchange rates for ebook {self.ebook_data[ebook]["name"]} released on {release_date} and currency {self.ebook_data[ebook]["curr"]} up to {max_days_ago} days before. Status code {nbp_request.status_code}.'
                    )
                    NBP_erros += 1
                else:
                    self.ebook_data[ebook]["fromNBP"]["rate"] = nbp_request.json()[
                        "rates"
                    ][0]["mid"]
                    self.ebook_data[ebook]["fromNBP"]["tableNo"] = nbp_request.json()[
                        "rates"
                    ][0]["no"]
                    self.ebook_data[ebook]["fromNBP"]["pricePLN"] = (
                        self.ebook_data[ebook]["price"]
                    ...
                    self.exchange_rates.loc[len(self.exchange_rates)] = (
                        new_exchange_rate
                    )
                    self.exchange_rates.to_csv("exchange_rates.csv", index=False)
    ...
    def save_ebook_data_to_json(self):
        ...
        print(f'\n{"="*10} Saving data to JSON file {"="*24}')
...
if __name__ == "__main__":
    ...
    ebook_price_converter = EbookPriceConverter(
        ebook_list_path=ebook_list_path,
